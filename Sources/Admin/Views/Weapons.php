...
if(verif_access("Admin"))
	{
		...
		{
			$Weapon_List = list_db("ref_list_t",array(
				'table' => 'Caranille_Items' ,
				'ID' => 'Item_Type',
				'ref' => implode("','",$array_weapon_type)
			));
		...
		if (request_confirm('Second_Edit'))
		{
			...
			$Weapon = get_db("edit_admin",array(
				'table' => 'Caranille_Items' ,
				'ID' => 'Item_ID',
				'value' => $Item_ID
			));
...
?>
            <p>Supprimer definitivement ?</p>
                <form method="POST" action="<?php echo get_link("Weapons","Admin") ?>">
...
<?php
		...
		else // 		if (empty($_POST['Edit']) && empty($_POST['Second_Edit']) && (empty($_POST['Add'])))
		{
			...
			echo '<form method="POST" action="'.get_link("Weapons","Admin").'">';
