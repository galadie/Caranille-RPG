...
<form method="post" action="<?php echo get_link('Post','Forum',array('action'=>'repondre','t'=>$topic)) ?>" name="formulaire">
...
<h1>Nouveau topic</h1>
<form method="post" action="<?php echo get_link('Post','Forum',array('action'=>'nouveautopic','f'=>$forum)) ?>" name="formulaire">
...
case "edit":{ //Si on veut éditer le post
    ...
    if (verif_Access($Auth_Modo,true) || $Post_Createur === user_data('Account_ID'))
    {
        ...
        ?>
		<h1>Edition</h1>
        <form method="post" action="<?php echo get_link('Post','Forum',array('action'=>'edit','p'=>$post)) ?>" name="formulaire">
...
case "delete":{ //Si on veut supprimer le post
    ...
    if (verif_Access($Auth_Modo,true) || $Post_Createur === user_data('Account_ID'))
    {
        ...
        echo'<p><a href="'.get_link('Post','Forum',array('action'=>'delete','p'=>$post)).'">Oui</a> ou <a href="'.get_link('Main','Forum').'">Non</a></p>';
    }
}break;
?>
