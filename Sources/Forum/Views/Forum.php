...
$query=list_db('request_forum_topic',array('forum'=>$forum));
//echo $r ;
...
for ($i = 1 ; $i <= $nombreDePages ; $i++)
    echo ($i == $numpage) ? $i :'<a href="'.get_link('Forum','Forum',array('f'=>$forum,'page'=>$i)).'">'.$i.'</a>';
echo '</p>';
...
echo'<a href="'.get_link('Poster','Forum',array('action'=>'nouveautopic','f'=>$forum)).'">&plusmn;&nbsp;&nbsp;Nouveau Topic</a>';
        ...
        foreach( $query as $data)
        {
			extract(stripslashes_r($data));
			   ...
			   //<strong>'.$Topic_Genre.' : </strong>
                ...
                <td id="titre">
                <strong><a href="'.get_link('Topic','Forum',array('t'=>$Topic_ID)).'" title="Topic commencé à '.$d->format('H\hi \l\e d M,y').'">'.$Topic_Titre.'</a></strong></td>
		...
		$numpage = ceil($nbr_Post / $nombreDeMessagesParPage);
			   ...
			   $d = new Datetime($Post_Time);
				...
				if(!is_null($Topic_Last_Post) && !empty($Topic_Last_Post) && $Topic_Last_Post!="")
					...
					A <a href="'.get_link('Topic','Forum',array('t'=>$Topic_ID,'page'=>$numpage)).'#p_'.$Topic_Last_Post.'">'.$d->format('H\hi \l\e d M y').'</a>';
