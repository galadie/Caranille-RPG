<?php
	...
	if(isset($ended) && $ended == true ) ////(request_confirm('Confirm')  or   )
	{
	...
	if(isset($order) && $order == true )//(request_confirm('Order') or   )
	{
		...
		foreach ($Order_list as $Order )
		{
			...
			$html_order[$Order_ID] .=  '<form method="POST" action="'.get_link('Members','Register').'">';
	...
	if(isset($record) && $record == true)// (empty($_POST) && empty($_GET) ) ////['Register']))
	{	
